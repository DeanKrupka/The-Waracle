 ...
 */
contract BookOfLoreMock is Ownable(msg.sender), EIP712 {
    struct Lore {
        address creator;
    ...
    modifier onlyAllowedTokenContract(address tokenContract) {
        require(loreTokenContractAllowlist[tokenContract], "tokenContract is not on the allowlist");
        _;
    }

    constructor() EIP712("BookOfLore", "1") {}

    function numLore(address tokenContract, uint256 tokenId) public view returns (uint256) {
        return tokenLore[tokenContract][tokenId].length;
    }

    function loreFor(address tokenContract, uint256 tokenId) public view returns (Lore[] memory) {
        return tokenLore[tokenContract][tokenId];
    }

    function loreAt(address tokenContract, uint256 tokenId, uint256 startIdx, uint256 endIdx)
        public
        view
        returns (Lore[] memory)
    {
    ...
    function setLoreTokenAllowlist(address tokenContract, bool isListed) public onlyOwner {
        loreTokenContractAllowlist[tokenContract] = isListed;
    }

    function setScribeAllowlist(address scribeAddress, bool isScribe) public onlyOwner {
    ...
    function addLore(
        ...
        require(tokenOwner == _msgSender(), "Owner: caller is not the token owner");
        tokenLore[tokenContract][tokenId].push(Lore(_msgSender(), parentLoreId, nsfw, false, loreMetadataURI));
        emit LoreAdded(tokenContract, tokenId, tokenLore[tokenContract][tokenId].length - 1);
    }
    ...
    function addLoreWithSignature(
        ...
        require(signer == tokenOwner, "addLoreWithSignature: signature is not the current token owner");

        require(numLore(tokenContract, tokenId) == loreId, "addLoreWithSignature: loreId is stale");

        tokenLore[tokenContract][tokenId].push(Lore(signer, parentLoreId, nsfw, false, loreMetadataURI));
        emit LoreAdded(tokenContract, tokenId, tokenLore[tokenContract][tokenId].length - 1);
    }
    ...
    function updateLoreMetadataURI(
        ...
        // is lore creator
        require(
            tokenLore[tokenContract][tokenId][loreIdx].creator == _msgSender(), "Owner: caller is not the Lore creator"
        );
        ...
        tokenLore[tokenContract][tokenId][loreIdx].loreMetadataURI = newLoreMetadataURI;

        emit LoreUpdated(tokenContract, tokenId, loreIdx);
    }

    function updateLoreNSFW(address tokenContract, uint256 tokenId, uint256 loreIdx, bool newNSFW)
        public
        onlyAllowedTokenContract(tokenContract)
    {
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);

        require(
            (tokenLore[tokenContract][tokenId][loreIdx].creator == _msgSender() && tokenOwner == _msgSender())
                || (owner() == _msgSender()),
    ...
    function strikeLore(address tokenContract, uint256 tokenId, uint256 loreIdx, bool newStruck)
        public
        onlyAllowedTokenContract(tokenContract)
    {
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
